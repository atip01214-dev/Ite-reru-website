...
import type { NewsItem, NewsCategory } from "@/lib/api";
import SectionHeading from "./SectionHeading";

const ALL_CATEGORIES = ["ทั้งหมด", "ข่าวประชาสัมพันธ์", "กิจกรรมนักศึกษา", "งานวิจัยและผลงาน"] as const;
...
export default function News({ initialNews }: NewsProps) {
  const [filter, setFilter] = useState<(typeof ALL_CATEGORIES)[number]>("ทั้งหมด");
  const filtered = filter === "ทั้งหมด" ? initialNews : initialNews.filter((n) => n.category === filter);
