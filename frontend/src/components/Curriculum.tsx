...
import type { ProgramData, CourseData, OtherProgramData, CareerData } from "@/lib/api";
...
interface CurriculumProps {
  ...
  courses: CourseData[];
  otherPrograms: OtherProgramData[];
  careers: CareerData[];
}

export default function Curriculum({ programs, courses, otherPrograms, careers }: CurriculumProps) {
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-8 grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-6">
          {courses.map((course, i) => {
            const Icon = courseIcons[course.icon] ?? CodeXml;
            return (
              <motion.div
                key={course.id}
